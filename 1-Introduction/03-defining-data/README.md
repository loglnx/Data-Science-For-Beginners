...
- What data is
  数据是用来发现新知识和支持有根据的决策的事实、信息、观察和测量。
- How data is described
  数据可以根据它的特征和来源进行描述、分类和归类。
- How data is classified and categorized
  数据的结构描述了它是如何组织的，可以是结构化、非结构化或半结构化的。数据的类型描述了它是如何观察或测量的，可以是定量的或定性的。
- Where data can be found
  数据的来源是数据产生或存储的初始位置，可以是数据库、文件、互联网或应用程序编程接口等。
